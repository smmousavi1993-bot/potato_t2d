...
array carbdenca {16}  carbden86ca carbden86ca carbden90ca carbden90ca carbden94ca carbden94ca carbden98ca carbden98ca carbden02ca carbden02ca carbden06ca carbden06ca carbden10ca carbden10ca carbden14ca carbden14ca;
array protdenca {16}  protden86ca protden86ca protden90ca protden90ca protden94ca protden94ca protden98ca protden98ca protden02ca protden02ca protden06ca protden06ca protden10ca protden10ca protden14ca protden14ca;
array tfatdenca {16}  tfatden86ca tfatden86ca tfatden90ca tfatden90ca tfatden94ca tfatden94ca tfatden98ca tfatden98ca tfatden02ca tfatden02ca tfatden06ca tfatden06cat tfatden10ca tfatden10ca tfatden14ca tfatden14ca;
...
array confca   {16}   conf86ca conf86ca conf90ca conf90ca conf94ca conf94ca conf98ca conf98ca conf02ca conf02ca conf06ca conf06ca conf10ca conf10ca conf14ca conf14ca ;
...
*****************************************************************************/
   ...
   array state {16} state10_88_num state10_88_num state10_90_num state10_92_num state10_94_num state10_96_num state10_98_num state10_00_num
        state10_02_num state10_04_num state10_06_num state10_08_num state10_10_num state10_12_num state10_14_num state10_16_num;
...
agecon=age{i};
	...
	if agecon<65 then age65=0;
     ...
     catsmk=smm{i};
     if catsmk=. then catsmk=.;
     %indic3(vbl=catsmk, prefix=catsmkc,min=2,max=5, reflev=1, missing=., usemiss=1,
           label1='Never smoke',
           label2='Past smoker',
...
%indic3(vbl=actcc, prefix=actc, min=2, max=5, reflev=1, missing=., usemiss=0);
      ...
      /****** Indicator Alcohol consumption ******/
       alcocon=alcoca{i}; /***continuous variable, g/day***/ 
            ...
            if alcoca{i} = . then alcq{i} = .;
            else if alcoca{i}=0.0                     then alcq{i}=1;
            else if alcoca{i}>0.0  and alcoca{i}<5.0   then alcq{i}=2;
            else if alcoca{i}>=5.0 and alcoca{i}<10.0  then alcq{i}=3;
            else if alcoca{i}>=10.0 and alcoca{i}<15.0 then alcq{i}=4;
            else if alcoca{i}>=15.0 and alcoca{i}<30.0 then alcq{i}=5;
            else if alcoca{i}>=30.0                   then alcq{i}=6;
     
        alc=alcq{i};
       %indic3(vbl=alc, prefix=alcc, min=2, max=6, reflev=1, missing=., usemiss=0,
                     label1='NON-DRINKERS',      
...
keep id period dtdth agecon agegp bmidi agedi 
    ...
    dead tdead dbsymp tdbsymp irtv irty
     ...
     prmeatcav urmeatcav trmeatcav poultrcav fishalcav regeggcav hdairycav ldairycav soyprocav nutsalcav legumecav fruitscav vegealcav nstvegcav stavegcav tdairycav coffeecav nutlegcav bwricecav wtricecav SSBcav confcav
     ...
     catrace ethnic hisp14 smkdi hbcbase protdencav tfatdencav carbdencav
     calorv calorcav alcov alcocav gidv gidcav glv glcav
     white2 &white_ &bmic_ &alcc_ &actc_ asp famhxmi mlvit bmicon actcon &bmic9_ bmic9 actcc catsmk alc bmisq
...
proc means data=two n nmiss mean stddev min max ;
     var tpotatcav bbmpotcav fpotatcav chipsscav prmeatcav urmeatcav trmeatcav poultrcav fishalcav regeggcav hdairycav ldairycav nutsalcav legumecav fruitscav vegealcav nstvegcav stavegcav whgrnscav rfgrnscav;
  run;
...
proc means data=two n mean median std min max;
    class white2; /* Groups the summary by 0 and 1 */
    var tpotatcav bbmpotcav fpotatcav;
run;

proc means data=two nmiss; var eth3g;run;
...
varlist=calorcav calorv transfcav alcov alcocav prmeatv urmeatv trmeatv poultrv fishalv regeggv hdairyv ldairyv tdairyv nutlegv legumev fruitsv soyprov nstvegv stavegv bwricev wtricev
        ...
        gidv whgrnsv rfgrnsv confcav nSES,
numquant=5,
mscore=T,
quantname=q,
outdat=four,
...
/*proc means data=two n nmiss mean median std min max; var eth3g nSES bmic9 actcc catsmk alc
calorcav calorv  alcov alcocav
...
proc freq data=four;tables states white2 catsmk &catsmkc_ &bmic_ &alcc_ &actc_ mlvit age65 aheinoptd dbfh antihbp antihtc hbp86 diabcase dbsymp hbcbase eth3g &strrace_;run;
proc univariate data=four;var agecon calorcav transfcav calorv alcov alcocav bmicon bmisq actcon tpotatcav bbmpotcav fpotatcav chipsscav prmeatcav urmeatcav trmeatcav poultrcav fishalcav regeggcav hdairycav ldairycav tdairycav soyprocav nutsalcav legumecav fruitscav vegealcav nstvegcav stavegcav gidcav SSBcav bwricecav wtricecav
                            prmeatv urmeatv trmeatv poultrv fishalv regeggv hdairyv ldairyv tdairyv soyprov nutsalv legumev fruitsv vegealv nstvegv stavegv gidv whgrnsv rfgrnsv bwricev wtricev SSBv tpotatv bbmpotv fpotatv chipssv confcav
...
%indic3(vbl=cfpotatcav, prefix=cfpotatcav, reflev=0, min=1, max=4, missing=., usemiss=0);
   ...
   if cfpotatcav=3 then medcfpotatcav=0.2560000;
...
%indic3(vbl=cbbmpotcav, prefix=cbbmpotcav, reflev=0, min=1, max=3, missing=., usemiss=0);
   ...
   if cbbmpotcav=2 then medcbbmpotcav=0.3212500;
