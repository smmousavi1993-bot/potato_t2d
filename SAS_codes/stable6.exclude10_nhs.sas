...
if cmarry ne 1 then cmarry=0;
   ...
   %exclude(0 lt dtdxdiab le irt94);         /* diabetes in the first 10 years after baseline which is 1984 */
...
proc means data=two n nmiss mean stddev min max ;
     var tpotatcav bbmpotcav fpotatcav chipsscav prmeatcav urmeatcav trmeatcav poultrcav fishalcav regeggcav hdairycav ldairycav nutsalcav legumecav fruitscav vegealcav nstvegcav stavegcav whgrnscav rfgrnscav;run;
...
proc means data=two n mean median std min max;
    class actbi; /* Groups the summary by 0 and 1 */
    var act86m act88m  act92m act94m act96m act98m act00m  act04m  act08m  act12m act14m;
run;
...
%indic3(vbl=cchipsscav, prefix=cchipsscav, reflev=0, min=1, max=4, missing=., usemiss=0);
   ...
   if cchipsscav=4 then medcchipsscav=0.6066667;
  ...
  %indic3(vbl=cbbmpotcav, prefix=cbbmpotcav, reflev=0, min=1, max=3, missing=., usemiss=0);
   ...
   if cbbmpotcav=3 then medcbbmpotcav=0.6000000;
...
ods rtf file="stable6.exclude10_nhs.rtf";  
...
ods rtf file="stable6.exclude10_bbm_nhs.rtf";  
proc print data=scr noobs;
...
ods rtf file="stable6.exclude10_pserv_nhs.rtf";  
